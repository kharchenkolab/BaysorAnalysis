...
```julia
@time df_spatial, gene_names = B.load_df("/home/vpetukhov/spatial/Benchmarking/run_results/merfish_moffit_watershed_prior/segmentation.csv");
@time dapi_arr = Float16.(Images.load("/home/vpetukhov/data/spatal/merfish_moffit/dapi_merged.tiff"));
@time dapi_seg_labels = B.load_segmentation_mask("/home/vpetukhov/data/spatal/merfish_moffit/dapi_merged_watershed.tif");
...
df_spatial.cell .= B.load_df("/home/vpetukhov/spatial/Benchmarking/run_results/merfish_moffit/segmentation.csv")[1].cell;
```
...
```julia
B.plot_cell_borders_polygons(@where(df_spatial, :x .> 2000, :x .< 4000, :y .> 6000, :y .< 8000), color=:color, size=(1000, 1000), ticks=true)
```

```julia
xs, ys = (3250, 3850), (7000, 7650)
size_mult = 4.0
ms = 2.0
plts = [B.plot_subset(df_spatial, dapi_arr, xs, ys; ms=ms, alpha=0.75, dapi_alpha=0.5, cell_col=cs, noise=true, #color_col=cs,
        bandwidth=10.0, grid_step=3.0, min_border_length=20, dens_threshold=1e-3, ticks=true, size_mult=size_mult, 
...
# Plots.savefig(plt, "./plots/merfish.png")
Plots.closeall()
plt
```
