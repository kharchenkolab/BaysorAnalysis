...
```julia
@time df_spatial, gene_names = B.load_df("../run_results/spacejam2/allen_sm_fish/no_dapi/segmentation.csv");
df_spatial[!, :x] = round.(Int, 10 .* (df_spatial.x .- minimum(df_spatial.x)));
df_spatial[!, :y] = round.(Int, 10 .* (df_spatial.y .- minimum(df_spatial.y)));
```

```julia
@time paper_seg_labels = Float64.(Images.load("/home/vpetukhov/data/spatal/SpaceJam2Full/allen_sm_fish/segmentation_labels_from_json.tiff"));
...
```julia
@time dapi_arr = Float64.(Images.load("/home/vpetukhov/data/spatal/SpaceJam2Full/allen_sm_fish/dapi_merged.tiff"));
```
...
for df in (qc_per_cell, qc_per_cell_paper)
    ...
    deleterows!(df, findall(df.n_transcripts .< 30));
end
...
```julia
match_thresholds = 0.02:0.01:1.0
match_nums = [sum(any(((contingency_table ./ sum(contingency_table, dims=1)) .> mt) .& ((contingency_table ./ sum(contingency_table, dims=2)) .> mt), dims=1)) for mt in match_thresholds];
...
New 'population': Fezf2+ Pde1a- Mpped1-:

```julia
for ci in qc_per_cell.cell_id[match_noise][cell_ord_n[250:300]][1:5:20]
...
### Visualize large cells

```julia
for ci in qc_per_cell.cell_id[qc_per_cell.n_transcripts .> 800]
    display(B.plot_comparison_for_cell(df_spatial, ci, paper_seg_labels, dapi_arr; ms=4.0, bandwidth=10.0))
end
```
...
noise_expr = Baysor.prob_array(df_spatial.gene[df_spatial.cell .== 0], max_value=length(gene_names));
...
for t_cell_id in partially_matching_ids
    ...
    match_expr = Baysor.prob_array(t_df.gene[t_df.cell_paper .== t_mcp], max_value=length(gene_names));
    non_match_expr = Baysor.prob_array(t_df.gene[t_df.cell_paper .!= t_mcp], max_value=length(gene_names));
...
noise_expr = Baysor.prob_array(df_spatial.gene[df_spatial.cell_paper .== 0], max_value=length(gene_names));
...
for t_cell_id in partially_matching_ids_paper
    ...
    match_expr = Baysor.prob_array(t_df.gene[t_df.cell .== t_mcp], max_value=length(gene_names));
    non_match_expr = Baysor.prob_array(t_df.gene[t_df.cell .!= t_mcp], max_value=length(gene_names));
...
for (ci,mc,of) in collect(zip(partially_matching_ids, match_cors, cur_overlaps))[match_cors .< 0.25]
    display(B.plot_comparison_for_cell(df_spatial, ci, paper_seg_labels, dapi_arr; ms=4.0, bandwidth=10.0, 
